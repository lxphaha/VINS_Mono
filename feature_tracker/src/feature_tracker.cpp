...
void FeatureTracker::setMask() {
  ...
  // 每个特征点进去，在白masks上会黑园使其均匀化
  for (auto &it : cnt_pts_id) {
    if (mask.at<uchar>(it.second.first) == 255) {
      forw_pts.push_back(it.second.first);
      ids.push_back(it.second.second);
      track_cnt.push_back(it.first);
      // ? opencv 函数，把周围一个圆内置为零，不允许别的特帧点存在 可以改进
      cv::circle(mask, it.second.first, MIN_DIST, 0, -1);
    }
  }
}

// 把新提取的点加入容器，id给-1作为区分
...
void FeatureTracker::readImage(const cv::Mat &_img, double _cur_time) {
  ...
  forw_pts.clear(); // 清空当前帧特征点的信息

  if (cur_pts.size() > 0) // 上一帧有特征点，就可以光流追踪了
  {
    TicToc t_o;
    vector<uchar> status;
    vector<float> err;
    // Step 1 通过opencv光流追踪给的状态位剔除outlier,没有提特征点
    // status vector 0 或 1
    // ?  LK参数
    cv::calcOpticalFlowPyrLK(cur_img, forw_img, cur_pts, forw_pts, status, err, cv::Size(21, 21), 3);
    // OpticalFlowMultiLevel(cur_img, forw_img, cur_pts, forw_pts, status, true);

    for (int i = 0; i < int(forw_pts.size()); i++)
      // Step 2 通过图像边界剔除outlier
      if (status[i] && !inBorder(forw_pts[i]))
        status[i] = 0;              // 追踪状态检查在不在图像范围内
    reduceVector(prev_pts, status); // 没用到
    reduceVector(cur_pts, status);
    reduceVector(forw_pts, status);
    reduceVector(ids, status);
    reduceVector(cur_un_pts, status); // 去畸变
...
void FeatureTracker::rejectWithF() {
  ...
  if (forw_pts.size() >= 8) {
    ...
    vector<uchar> status;
    // opencv接口计算本质矩阵，对极约束的outlier剔除，虚拟焦距不影响本质矩阵
    // 本质矩阵定义 https://zhuanlan.zhihu.com/p/500798616
...
// 当前帧所有点统一去畸变，同时计算特征点速度，用于后续时间戳标定
