...
double first_image_time;       // 第一帧或重启后的时间戳
...
void img_callback(const sensor_msgs::ImageConstPtr& img_msg) {
    ...
    // 检查时间戳是否正常的，这里认为超过一秒或者小于当前帧就异常
    // 图像时间差太多光流追踪就会失败，这里没有描述子匹配，对时间戳要求高
    ...
    // frequency control
    // 控制后端发送频率
    if (round(1.0 * pub_count / (img_msg->header.stamp.toSec() - first_image_time)) <= FREQ)  // 保证发送给后端的不超过这个频率（10hz）
    {
        PUB_THIS_FRAME = true;
        // reset the frequency control
        // 这段时间的频率与设定的频率十分接近，就认为这段时间很棒，重启一下，避免delta_t太大，需要定期更新
    ...
    for (unsigned int i = 0;; i++) {
        ...
        for (int j = 0; j < NUM_OF_CAM; j++)
            // !! |=是啥意思?
            if (j != 1 || !STEREO_TRACK)
                completed |= trackerData[j].updateID(i);  // 单目情况下可以直接用=号
        if (!completed)
            break;
    }
    ...
    if (PUB_THIS_FRAME) {
        ...
        if (!init_pub) {
            ...
            pub_img.publish(feature_points);  // 前端特征点的消息通过这个发布
        ...
        if (SHOW_TRACK) {
            ...
            // 显示彩色 robag本身是灰色的
            //   ptr = cv_bridge::toCvCopy(img_msg,sensor_msgs::image_encodings::BGR8); cv::imshow("vis", ptr->image);
...
int main(int argc, char** argv) {
    ...
    ros::console::set_logger_level(ROSCONSOLE_DEFAULT_NAME, ros::console::levels::Info);  // 设置ros log级别不用在意
    ...
    // 注册一些publisher
    pub_img = n.advertise<sensor_msgs::PointCloud>("feature", 1000);  // 实际发出的是 /feature_tracker/feature
    ...
    */
    ros::spin();  // spin代表节点开始查询topic是否接受
    return 0;
}
