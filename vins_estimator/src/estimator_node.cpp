...
void predict(const sensor_msgs::ImuConstPtr &imu_msg) {
  ...
  // 上一时刻世界坐标系下加速度值
  Eigen::Vector3d un_acc_0 = tmp_Q * (acc_0 - tmp_Ba) - estimator.g; // 减去重力

  // 中值陀螺仪的结果
  Eigen::Vector3d un_gyr = 0.5 * (gyr_0 + angular_velocity) - tmp_Bg;
  // 更新姿态
  tmp_Q = tmp_Q * Utility::deltaQ(un_gyr * dt); // 角度转换为四元数
  // 当前时刻世界坐标下的加速度值
...
/***
 * @brief     获得匹配好的图像imu组 一帧图像对应多帧imu
 * @return    {*}
 */
std::vector<std::pair<std::vector<sensor_msgs::ImuConstPtr>, sensor_msgs::PointCloudConstPtr>> getMeasurements() {
  ...
  while (true) {
    ...
    while (imu_buf.front()->header.stamp.toSec() < img_msg->header.stamp.toSec() + estimator.td) {
      IMUs.emplace_back(imu_buf.front()); // c++11特性： emplace_back push_back
      imu_buf.pop();
    }
  ...
  while (true) {
    ...
    for (auto &measurement : measurements) {
      ...
      for (unsigned int i = 0; i < img_msg->points.size(); i++) {
        ...
        double x = img_msg->points[i].x; // 去畸变归一化坐标
