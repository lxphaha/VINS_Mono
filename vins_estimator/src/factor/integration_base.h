...
class IntegrationBase {
   public:
    ...
    /***
     * @brief    预积分构造函数
     * @param[in] _acc_0 初始时刻的加速度
     * @param[in] _gyr_0 初始时刻的角速度
     * @param[in] _linearized_ba 加速度的零偏
     * @param[in] _linearized_bg 角速度的零偏
     * @return    {*}
     */
    IntegrationBase(const Eigen::Vector3d& _acc_0,
        ...
        : acc_0{_acc_0},  // 初始时刻的加速度
          ...
          jacobian{Eigen::Matrix<double, 15, 15>::Identity()},  // jac矩阵
          covariance{Eigen::Matrix<double, 15, 15>::Zero()},    // 协方差
          sum_dt{0.0},
    ...
    {
        // 噪声的协方差:k时刻加速度噪声、陀螺仪噪声、k+1时刻加速度噪声、陀螺仪噪声、加速度零偏、陀螺仪零偏 18*18
     ...
     * @param[in] _acc_1 k+1时刻加速度
     * @param[in] _gyr_1 k+1时刻角速度
     * @param[in] delta_p 位置p
     * @param[in] delta_q 角度q
     * @param[in] delta_v 速度v
     * @param[in] linearized_ba 加速度零偏
     * @param[in] linearized_bg 角速度零偏
     * @param[in] result_delta_p 更新位置p
     * @param[in] result_delta_q 更新角度q
     * @param[in] result_delta_v 更新速度v
    ...
    void midPointIntegration(double _dt,
        ...
        result_delta_q = delta_q * Quaterniond(1, un_gyr(0) * _dt / 2, un_gyr(1) * _dt / 2, un_gyr(2) * _dt / 2);  // 省略归一化
    ...
    Eigen::Matrix<double, 18, 18> noise;  // 噪声协方差矩阵 构造函数里初始化，剩下的不变
