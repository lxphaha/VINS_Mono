...
/***
 * @brief     对imu数据处理，包括更新预积分量和提供优化状态量的初始值构造
 * @param[in] dt
...
void Estimator::processIMU(double dt, const Vector3d &linear_acceleration, const Vector3d &angular_velocity) {
  ...
  if (frame_count != 0) {
    ...
    // 又是一个中值积分，更新滑窗中的状态量，本质是给非线性优化提供可信的初始值
    int j = frame_count;
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const std_msgs::Header &header) {
  ...
  // Step 1 将特征点信息加到f_manger这个特征点管理器中，同时进行是否关键帧的检查 两种边缘化的策略
  if (f_manager.addFeatureCheckParallax(frame_count, image, td))
    // 如果上一帧是关键帧，则滑窗中最老的帧就要被移除滑窗
  ...
  // all_image_frame用于做初始化相关操作，他保留滑窗起始到当前的所有帧
  // 有一些帧因为不是KF,被MARGIN_SECOND_NEW，但及时较新的帧被margin,他也会保留在这容器中，因为初始化要求使用所有帧，而非只要关键帧
  ImageFrame imageframe(image, header.stamp.toSec());
  imageframe.pre_integration = tmp_pre_integration;
  // 这里是简单的把图像和预积分绑定在一起，这里预积分是两帧之间的，滑动窗口实际上是两个KF之间的
  // 实际上是用于准备来初始化的相关数据
  ...
  if (solver_flag == INITIAL) {
    if (frame_count == WINDOW_SIZE) // 有足够帧数
    {
      ...
      if (result) // 初始化成功进行一次非线性优化
      {
        ...
        slideWindow(); // 不成功则进行滑动窗操作
    } else
      frame_count++;
  } else // slover_flag = NON_LINEAR，进行非线性优化
  {
    TicToc t_solve;
    solveOdometry();
    ...
    TicToc t_margin;
    slideWindow();              // 滑动窗口函数
    f_manager.removeFailures(); // 去除失败的点并发布关键点位置
...
bool Estimator::initialStructure() {
  ...
  for (auto &it_per_id : f_manager.feature) {
    int imu_j = it_per_id.start_frame - 1;                 // 这个跟imu无关，就是存储观测特征点帧的索引
    SFMFeature tmp_feature;                                // 用来后续做sfm
    tmp_feature.state = false;                             // 状态（是否被三角化）
    tmp_feature.id = it_per_id.feature_id;                 // 特征点的id
    for (auto &it_per_frame : it_per_id.feature_per_frame) // 对于当前特征点在每一帧的坐标
    {
      imu_j++;                             // 帧编号+1
  ...
  // step2只针对KF进行sfm,初始化需要对all_image_frame中所有元素，因此下面通过KF来求解其他非KF的位姿
  map<double, ImageFrame>::iterator frame_it;
  ...
  for (int i = 0; frame_it != all_image_frame.end(); frame_it++) {
    ...
    if ((frame_it->first) == Headers[i].stamp.toSec()) {
      ...
      frame_it->second.T = T[i];                                         // 初始化不用估计外参
      i++;
      continue;
    }
    if ((frame_it->first) > Headers[i].stamp.toSec()) {
    ...
    for (auto &id_pts : frame_it->second.points) {
      ...
      // 单目 这里id_pts.second为1
      for (auto &i_p : id_pts.second) {
        it = sfm_tracked_points.find(feature_id);
        if (it != sfm_tracked_points.end()) // 有对应的三角化的3d点
        {
...
bool Estimator::visualInitialAlign() {
  ...
  // 将Ps、Vs、depth尺度缩放后从l帧转变为相对于c0帧图像坐标系下
  double s = (x.tail<1>())(0);
  ...
  // 通过将重力旋转到z轴上，得到世界坐标系与摄像坐标系c0之间的旋转矩阵rot_diff
...
bool Estimator::relativePose(Matrix3d &relative_R, Vector3d &relative_T, int &l) {
  ...
  for (int i = 0; i < WINDOW_SIZE; i++) {
    ...
    if (corres.size() > 20) // 归一化坐标point(x,y,不需要z)
    {
      ...
      // 有足够的视差在通过本质矩阵恢复第i帧和最后一帧的R t  T_i_last
      // 判断是否满足初始化条件：视差>30
...
void Estimator::optimization() {
  ...
  // Step 2 添加各种待优化量X-位姿优化量
  for (int i = 0; i < WINDOW_SIZE + 1; i++) // 包括最新的11帧
  {
    ceres::LocalParameterization *local_parameterization = new PoseLocalParameterization();
    problem.AddParameterBlock(para_Pose[i], SIZE_POSE, local_parameterization);
    problem.AddParameterBlock(para_SpeedBias[i], SIZE_SPEEDBIAS);
  }
  // Step 3 添加各种待优化量X-相机外参
  ...
  // Step 4  添加各种待优化量X——IMU——image时间同步误差
  ...
  if (marginalization_flag == MARGIN_OLD) {
    ...
    // 先验误差会一直保存，而不是使用一次
    // 如果上一次边缘化的信息存在
    // 要边缘化的参数块是para_Pose[0] para_SpeedBias[0]以及para_Feature[feature_index](滑窗内的第feature_index个点的逆深度)
    if (last_marginalization_info) {
      vector<int> drop_set;
    ...
    // Step 3 加入这次要marg的视觉信息
    {
      // 只加入起始帧是旧帧且观测次数大于2的Features
      int feature_index = -1;
      for (auto &it_per_id : f_manager.feature) {
        it_per_id.used_num = it_per_id.feature_per_frame.size();
        if (!(it_per_id.used_num >= 2 && it_per_id.start_frame < WINDOW_SIZE - 2))
 ...
 // Step 4 将三个ResidualBlockInfo中的参数综合到marginalization_info中
    TicToc t_pre_margin;
